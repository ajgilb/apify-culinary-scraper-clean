...
function processEmails(emailsData) {
    ...
    try {
        ...
        const scoredEmails = processedEmails.map(email => {
            ...
            // Get the email value and clean it up
            let emailValue = email.value || email.email || '';
            // Trim whitespace and ensure it's properly formatted
...
async function getCompanyInfo(rawCompanyName, rawLocation = '') {
    ...
    for (const result of allResults) {
        ...
        for (const email of result.data.emails) {
            // Clean the email address to ensure proper format
            const cleanEmail = (email.email || '').trim();
            if (!cleanEmail) continue; // Skip empty emails
        ...
        for (const item of data) {
            ...
            if (item.emails && item.emails.length > 0) {
                ...
                const emailsCopy = validatedEmails.map((email, idx) => {
                    ...
                    // Make sure email is properly formatted (trim any whitespace)
                    const cleanEmail = (email.email || '').trim();
...
Best regards,`);
                    ...
                    // Make sure email is clean and properly formatted
                    const cleanEmail = (email.email || '').trim();
                    ...
                    const contactRow = [
                        ...
                        email.name || 'Unknown',
                        email.title || 'N/A',
                        cleanEmail, // Use the clean email
